...
public class InputHandler : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.DownArrow) || Input.mousePosition.y <= padding) //|| Input.mousePosition.y <= padding
        {
            transform.Translate(-Vector2.up * Time.deltaTime * speed, Space.World);
        }

        if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow) || Input.mousePosition.x <= padding) //|| Input.mousePosition.x <= padding
        {
